...
@Setter
@NoArgsConstructor
public class PartGroupCompareRow implements ComparableRow<PartGroupCompareRow> {

    private String partGroupStr;
    private PartGroupDTO partGroup;

    private Map<Long, DataSet> dataSets = new HashMap<>();
    private List<PartGroupCompareRow> children = new ArrayList<>();
    private Map<Long, Map<ApCompareGroup, Double>> table = new HashMap<>();
    private boolean isSum = false;

    PartGroupCompareRow(PartGroupDTO partGroup, String partGroupStr) {
        this.partGroup = partGroup;
        this.partGroupStr = partGroupStr;
    }
    ...
    @Override
    public void addChildRow(PartGroupCompareRow row) {
        children.add(row);
    }
    ...
    public Map<ApCompareGroup, Double> getWeights(Long vehicleConfigId) {
        if (table.containsKey(vehicleConfigId)) {
            ...
            for (PartGroupCompareRow costGroupCompareRow : getChildren()) {
                Map<ApCompareGroup, Double> childWeights = costGroupCompareRow.getWeights(vehicleConfigId);
