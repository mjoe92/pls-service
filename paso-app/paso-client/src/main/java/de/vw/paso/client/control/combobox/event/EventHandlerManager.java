package de.vw.paso.client.control.combobox.event;
...
public class EventHandlerManager implements EventDispatcher {
  ...
  public final <T extends Event> void addEventHandler(final EventType<T> eventType,
    ...
    final CompositeEventHandler<T> compositeEventHandler = createGetCompositeEventHandler(eventType);
  ...
  public final <T extends Event> void removeEventHandler(final EventType<T> eventType,
    ...
    final CompositeEventHandler<T> compositeEventHandler = (CompositeEventHandler<T>) eventHandlerMap.get(eventType);
  ...
  public final <T extends Event> EventHandler<? super T> getEventHandler(final EventType<T> eventType) {
    final CompositeEventHandler<T> compositeEventHandler = (CompositeEventHandler<T>) eventHandlerMap.get(eventType);
  ...
  private Event dispatchCapturingEvent(final EventType<? extends Event> handlerType, Event event) {
    final CompositeEventHandler<? extends Event> compositeEventHandler = eventHandlerMap.get(handlerType);
  ...
  private Event dispatchBubblingEvent(final EventType<? extends Event> handlerType, Event event) {
    final CompositeEventHandler<? extends Event> compositeEventHandler = eventHandlerMap.get(handlerType);
  ...
  private static Event fixEventSource(final Event event, final Object eventSource) {
  ...
  private static void validateEventHandler(final EventHandler<?> eventHandler) {
    if (eventHandler == null) {
      throw new NullPointerException("Event handler must not be null");
    }
  }

  private static void validateEventFilter(final EventHandler<?> eventFilter) {
    if (eventFilter == null) {
