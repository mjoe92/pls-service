...
public class AutoCompletePopup<T> extends PopupControl {
  ...
  public static class SuggestionEvent<TE> extends Event {
    ...
    public static final EventType<SuggestionEvent<?>> SUGGESTION = new EventType<>(
      "SUGGESTION" + UUID.randomUUID()); //$NON-NLS-1$
  ...
  public void show(Node node) {
    ...
    this.show(parent, parent.getX() + node.localToScene(0, 0).getX() + node.getScene().getX(),
      parent.getY() + node.localToScene(0, 0).getY() + node.getScene().getY() + node.getBoundsInParent().getHeight());
